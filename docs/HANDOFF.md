...
The release was saved in GitHub and deployed to Hostinger on September 22. Browser assets now have a release version in index.html to avoid stale cached UI. AI captions are enabled with the default $10 estimated allowance. One live text-only test reached OpenAI but was rejected with a billing/rate-limit response; the account billing page confirmed $0.00 API credits. The app recorded one attempt and $0 estimated spend. The user was asked to add API credits; successful live generation remains unverified. No social publication occurred.
